...
use dynlint_internal::{clone, env};
use std::path::Path;
...
fn ui() {
    ...
    dynlint_internal::cargo::build("clippy", false)
        .success()
        .unwrap();
    ...
    let mut command = dynlint_internal::cargo::test("clippy", false);
    command
        ...
        .envs([
            ...
            (env::DYNLINT_RUSTFLAGS, r#"--cfg feature="cargo-clippy""#),
        ])
        .args(["--test", "compile-test"]);

    // smoelius: Error messages like the following have occurred in Windows GitHub workflows:
    //   LINK : fatal error LNK1318: Unexpected PDB error; OK (0) 'D:\a\dynlint\dynlint\examples\
...
fn clippy_lints_dependency() -> Result<Dependency> {
    let metadata = dynlint_internal::cargo::current_metadata()?;

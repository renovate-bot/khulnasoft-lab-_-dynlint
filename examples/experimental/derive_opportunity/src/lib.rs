...
fn ui_at_least_one_field() {
    dynlint_testing::ui::Test::example(env!("CARGO_PKG_NAME"), "ui_at_least_one_field")
        .dynlint_toml("derive_opportunity.at_least_one_field = true")
        .run();
}

#[test]
fn ui_ignore() {
    dynlint_testing::ui::Test::example(env!("CARGO_PKG_NAME"), "ui_ignore")
        .dynlint_toml(r#"derive_opportunity.ignore = ["serde_derive::Deserialize"]"#)
        .run();
}

...
use dynlint_internal::{cargo::cargo_home, env, library_filename, Command};
...
mod test {
    ...
    use dynlint_internal::packaging::isolate;
    ...
    fn global_config() {
        ...
        dynlint_internal::cargo::build("dynlint-link", false)
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .success()
            .unwrap();

        dynlint_internal::cargo::init("package `global_config_test`", false)
            .current_dir(&package)
        ...
        std::process::Command::new("cargo")
            ...
            .stderr(
                predicate::str::is_match(
                    "error: linking with `[^`]*/target/debug/dynlint-link` failed",
                )
                .unwrap(),
            );
    }

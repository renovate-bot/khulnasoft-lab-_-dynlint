...
use dynlint_internal::{env, parse_path_filename, rustup::is_rustc};
use std::{
...
type DynlintVersionFunc = unsafe fn() -> *mut std::os::raw::c_char;
...
impl LoadedLibrary {
    fn register_lints(
        ...
        (|| unsafe {
            if let Ok(func) = self.lib.get::<DynlintVersionFunc>(b"dynlint_version") {
                let dynlint_version = CString::from_raw(func()).into_string()?;
                ensure!(
                    dynlint_version == DYNLINT_VERSION,
                    "`{}` has dynlint version `{}`, but `{}` was expected",
                ...
                bail!(
                    "could not find `dynlint_version` in `{}`",
                    self.path.to_string_lossy()
                );
            }
...
impl rustc_driver::Callbacks for Callbacks {
    fn config(&mut self, config: &mut rustc_interface::Config) {
        ...
        config.register_lints = Some(Box::new(move |sess, lint_store| {
            ...
            let dynlint_libs = env::var(env::DYNLINT_LIBS).ok();
            let dynlint_metadata = env::var(env::DYNLINT_METADATA).ok();
            let dynlint_no_deps = env::var(env::DYNLINT_NO_DEPS).ok();
            let dynlint_no_deps_enabled =
                dynlint_no_deps.as_ref().map_or(false, |value| value != "0");
            let cargo_primary_package_is_set = env::var(env::CARGO_PRIMARY_PACKAGE).is_ok();

            sess.parse_sess.env_depinfo.lock().insert((
                rustc_span::Symbol::intern(env::DYNLINT_LIBS),
                dynlint_libs.as_deref().map(rustc_span::Symbol::intern),
            ));
            sess.parse_sess.env_depinfo.lock().insert((
                rustc_span::Symbol::intern(env::DYNLINT_METADATA),
                dynlint_metadata.as_deref().map(rustc_span::Symbol::intern),
            ));
            sess.parse_sess.env_depinfo.lock().insert((
                rustc_span::Symbol::intern(env::DYNLINT_NO_DEPS),
                dynlint_no_deps.as_deref().map(rustc_span::Symbol::intern),
            ));

            if dynlint_no_deps_enabled && !cargo_primary_package_is_set {
                return;
            }
...
fn list_enabled() -> bool {
    env::var(env::DYNLINT_LIST).map_or(false, |value| value != "0")
}
...
pub fn dynlint_driver<T: AsRef<OsStr>>(args: &[T]) -> Result<()> {
...
fn rustflags() -> Vec<String> {
    env::var(env::DYNLINT_RUSTFLAGS).map_or_else(
        |_| Vec::new(),
...
fn paths() -> Vec<PathBuf> {
    (|| -> Result<_> {
        let dynlint_libs = env::var(env::DYNLINT_LIBS)?;
        serde_json::from_str(&dynlint_libs).map_err(Into::into)
    })()
    .unwrap_or_default()
}
...
fn rustc_args<T: AsRef<OsStr>, U: AsRef<str>, V: AsRef<Path>>(
    ...
    for path in paths {
        if let Some((name, _)) = parse_path_filename(path.as_ref()) {
            rustc_args.push(format!(r#"--cfg=dynlint_lib="{name}""#));
        } else {

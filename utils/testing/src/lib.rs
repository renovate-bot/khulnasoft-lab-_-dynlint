...
use dynlint_internal::{env, library_filename, rustup::is_rustc};
...
///
/// - `name` is the name of a Dynlint library to be tested. (Often, this is the same as the package
///   name.)
...
///
/// - `name` is the name of a Dynlint library to be tested.
...
fn initialize(name: &str) -> Result<&Path> {
    DRIVER
        .get_or_try_init(|| {
            ...
            dynlint_internal::cargo::build(&format!("library `{name}`"), false).success()?;

            // smoelius: `DYNLINT_LIBRARY_PATH` must be set before `dynlint_libs` is called.
            // smoelius: This was true when `dynlint_libs` called `name_toolchain_map`, but that is
            ...
            let dynlint_library_path = metadata.target_directory.join("debug");
...
#[doc(hidden)]
pub fn dynlint_libs(name: &str) -> Result<String> {
    let metadata = dynlint_internal::cargo::current_metadata().unwrap();
...
fn rustc_flags(metadata: &Metadata, package: &Package, target: &Target) -> Result<Vec<String>> {
    ...
    let output = {
        ...
        dynlint_internal::cargo::build(&format!("`{}` examples", package.name), false)
...
fn run_tests(driver: &Path, src_base: &Path, config: &ui::Config) {
    ...
    let _var = config
        .dynlint_toml
        .as_ref()
        .map(|value| VarGuard::set(env::DYNLINT_TOML, value));

...
pub(super) struct Config {
    ...
    pub(super) dynlint_toml: Option<String>,
}
...
impl Test {
    ...
    /// Set the `dynlint.toml` file's contents (for testing configurable libraries).
    pub fn dynlint_toml(&mut self, dynlint_toml: impl AsRef<str>) -> &mut Self {
        self.config.dynlint_toml = Some(dynlint_toml.as_ref().to_owned());
        self
    }
    ...
    fn run_immutable(&self) {
        ...
        match &self.target {
            ...
            Target::Example(example) => {
                ...
                let package =
                    dynlint_internal::cargo::package_with_root(&metadata, &current_dir).unwrap();

//! This crate provides macros for creating [Dynlint] libraries, and utilities for creating
...
//! # `dynlint_library!`
//!
//! The `dynlint_library!` macro expands to the following:
//!
//! ```rust,ignore
...
//!
//! #[no_mangle]
//! pub extern "C" fn dynlint_version() -> *mut std::os::raw::c_char {
//!     std::ffi::CString::new($crate::DYNLINT_VERSION)
//!         .unwrap()
//!         .into_raw()
//! }
//! ```
//!
//! If your library uses the `dynlint_library!` macro and the [`dynlint-link`] tool, then all you
...
//!
//! ```rust,ignore
//! dynlint_linting::dynlint_library!();
//!
...
//!     dynlint_linting::init_config(sess);
...
//! # Configurable libraries
//!
//! Libraries can be configured by including a `dynlint.toml` file in the target workspace's root
//! directory. This crate provides the following functions for reading and parsing `dynlint.toml`
//! files:
//!
//! - [`config_or_default`]
//! - [`config`]
...
//!
//! ```rust,ignore
//! dynlint_linting::impl_late_lint! {
//!     ...,
//!     LintName::new()
//! }
//!
...
//! }
//!
//! // Read the configuration from the `dynlint.toml` file, or use the default configuration if
//! // none is present.
//! impl LintName {
//!     pub fn new() -> Self {
//!         Self {
//!             config: dynlint_linting::config_or_default(env!("CARGO_PKG_NAME")),
//!         }
//!     }
//! }
//! ```
...
//! should include a call to `dynlint_linting::init_config`, as in the following example:
//!
//! ```rust,ignore
//! #[no_mangle]
//! pub fn register_lints(sess: &rustc_session::Session, lint_store: &mut rustc_lint::LintStore) {
...
//! [`dynlint_library!`]: #dynlint_library
...
//! [docs.rs documentation]: https://docs.rs/dynlint_linting/latest/dynlint_linting/
//! [docs.rs]: https://docs.rs/dynlint_linting/latest/dynlint_linting/
//! [dynlint]: ../..
//! [examples]: ../../examples
...
pub const DYNLINT_VERSION: &str = "0.1.0";

pub use paste;

// smoelius: Including `extern crate rustc_driver` causes the library to link against
// `librustc_driver.so`, which dynlint-driver also links against. So, essentially, the library uses
// dynlint-driver's copy of the Rust compiler crates.
#[macro_export]
macro_rules! dynlint_library {
    () => {
        #[allow(unused_extern_crates)]
        extern crate rustc_driver;
...
/// - If the target workspace's `dynlint.toml` file does not exist or does not contain key `name`,
...
/// Reads an entry from the workspace's `dynlint.toml` file as a raw `toml::Value`.
///
/// Returns:
...
/// Reads the target workspace's `dynlint.toml` file and parses it as a `toml::value::Table`.
///
/// Note: `init_config` or `try_init_config` must be called before `config_or_default`, `config`, or
/// `config_toml` is called. However, the `register_lints` function generated by `impl_late_lint`,
/// etc. includes a call to `init_config`.
pub fn try_init_config(sess: &rustc_session::Session) -> ConfigResult<()> {
    ...
    let value = if let Ok(value) = std::env::var(env::DYNLINT_TOML) {
        sess.parse_sess.env_depinfo.lock().insert((
            Symbol::intern(env::DYNLINT_TOML),
        ...
        //
        //          Caused by:
        //            failed to parse manifest at `D:\a\dynlint\dynlint\examples\general\await_holding_span_guard\Cargo.toml`
        //
        //          Caused by:
        ...
        match result {
            ...
            _ => {
                ...
                let dynlint_toml = workspace_root.join("dynlint.toml");

                if dynlint_toml.try_exists().map_err(|error| {
                    ConfigErrorInner::Io(format!("`try_exists` failed for {dynlint_toml:?}"), error)
                })? {
                    let value = read_to_string(&dynlint_toml).map_err(|error| {
                        ConfigErrorInner::Io(
                            format!("`read_to_string` failed for {dynlint_toml:?}"),
                            error,
                        )
                    })?;
                    sess.parse_sess
                        .file_depinfo
                        .lock()
                        .insert(Symbol::intern(dynlint_toml.as_str()));
                    Some(value)
                } else {
                    None
                }

...
fn one_name_multiple_toolchains() {
    ...
    patch_dynlint_template(tempdir.path(), CHANNEL_A, CLIPPY_UTILS_REV_A).unwrap();
    dynlint_internal::cargo::build(
        &format!("dynlint-template with channel `{CHANNEL_A}`"),
        false,
    )
    ...
    patch_dynlint_template(tempdir.path(), CHANNEL_B, CLIPPY_UTILS_REV_B).unwrap();
    dynlint_internal::cargo::build(
        &format!("dynlint-template with channel `{CHANNEL_B}`"),
        false,
    )
    .sanitize_environment()
    .current_dir(&tempdir)
    ...
    std::process::Command::cargo_bin("cargo-dynlint")
        .unwrap()
        .envs([(
            env::DYNLINT_LIBRARY_PATH,
            target_debug(tempdir.path()).unwrap(),
        )])
        .args(["dynlint", "list", "--all", "--no-metadata"])
        .assert()
        .success()
        .stdout(
...
fn patch_dynlint_template(path: &Path, channel: &str, clippy_utils_rev: &str) -> Result<()> {
...
fn one_name_multiple_paths() {
    ...
    dynlint_internal::cargo::build(
        &format!("dynlint-template in {:?}", tempdirs.0.path()),
        false,
    )
    .sanitize_environment()
    ...
    dynlint_internal::cargo::build(
        &format!("dynlint-template in {:?}", tempdirs.1.path()),
        false,
    )
    .sanitize_environment()
    ...
    std::process::Command::cargo_bin("cargo-dynlint")
        .unwrap()
        .envs([(env::DYNLINT_LIBRARY_PATH, paths)])
        .args(["dynlint", "list", "--all", "--no-metadata"])
        .assert()
        .success()
        .stdout(
...
fn relative_path() {
    ...
    dynlint_internal::cargo::build(&format!("dynlint-template in {:?}", tempdir.path()), false)
        .sanitize_environment()
        .current_dir(&tempdir)
        .success()
        .unwrap();
    ...
    for path in [
        ...
        std::process::Command::cargo_bin("cargo-dynlint")
            .unwrap()
            .current_dir(&tempdir)
            .envs([(env::DYNLINT_LIBRARY_PATH, &path)])
            .args(["dynlint", "list"])
            .assert()
            .success()
...
fn list_by_path() {
    ...
    std::process::Command::cargo_bin("cargo-dynlint")
        .unwrap()
        .args(["dynlint", "list", "--path", &path.to_string_lossy()])
        .assert()
        .success()
        .stdout(

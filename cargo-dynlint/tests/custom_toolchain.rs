// smoelius: As per `dynlint-link/src/main.rs`:
...
fn custom_toolchain() {
    ...
    patch_dynlint_template(tempdir.path(), &custom_toolchain).unwrap();

    dynlint_internal::cargo::test(
...
fn patch_dynlint_template(path: &Path, channel: &str) -> Result<()> {

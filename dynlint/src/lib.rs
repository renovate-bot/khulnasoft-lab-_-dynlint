#![allow(deprecated)]
#![cfg_attr(dynlint_lib = "general", allow(crate_wide_allow))]
...
use dynlint_internal::{
    driver as dynlint_driver, env, parse_path_filename, rustup::SanitizeEnvironment,
};
...
pub fn run(opts: &Dynlint) -> Result<()> {
    let opts = {
        if opts.force {
            warn(
                opts,
...
fn warn_if_empty(opts: &Dynlint, name_toolchain_map: &NameToolchainMap) -> Result<bool> {
...
fn resolve(opts: &Dynlint, name_toolchain_map: &NameToolchainMap) -> Result<ToolchainMap> {
...
fn list_lints(opts: &Dynlint, resolved: &ToolchainMap) -> Result<()> {
    for (toolchain, paths) in resolved {
        for path in paths {
            let driver = driver_builder::get(opts, toolchain)?;
            let dynlint_libs = serde_json::to_string(&[path])?;
            let (name, _) =
            ...
            command
                .envs([
                    (env::DYNLINT_LIBS, dynlint_libs.as_str()),
                    (env::DYNLINT_LIST, "1"),
                ])
                .args(["rustc", "-W", "help"])
                .success()?;
...
fn check_or_fix(opts: &Dynlint, resolved: &ToolchainMap) -> Result<()> {
    ...
    for (toolchain, paths) in resolved {
        ...
        let driver = driver_builder::get(opts, toolchain)?;
        let dynlint_libs = serde_json::to_string(&paths)?;
        #[cfg(not(feature = "metadata"))]
        let dynlint_metadata = None;
        #[cfg(feature = "metadata")]
        let dynlint_metadata = metadata::dynlint_metadata(opts)?;
        let dynlint_metadata_str = dynlint_metadata
        ...
        let mut command = if opts.fix {
            dynlint_internal::cargo::fix(&description)
        } else {
            dynlint_internal::cargo::check(&description)
        };
        ...
        command
            ...
            .envs([
                ...
                (env::DYNLINT_LIBS, &dynlint_libs),
                (env::DYNLINT_METADATA, &dynlint_metadata_str),
                (env::DYNLINT_NO_DEPS, if opts.no_deps { "1" } else { "0" }),
...
fn target_dir(opts: &Dynlint, toolchain: &str) -> Result<PathBuf> {
...
mod test {
    ...
    static OPTS: Lazy<Dynlint> = Lazy::new(|| Dynlint {
        no_metadata: true,
        ..Dynlint::default()
    });
    ...
    fn name_toolchain_map() -> NameToolchainMap<'static> {
        ...
        let metadata = dynlint_internal::cargo::current_metadata().unwrap();
        // smoelius: As of version 0.1.14, `cargo-llvm-cov` no longer sets `CARGO_TARGET_DIR`.
        // So `dynlint_library_path` no longer requires a `cfg!(coverage)` special case.
        let dynlint_library_path = join_paths([
        ...
        //
        //   error: internal compiler error: encountered incremental compilation error with shallow_lint_levels_on(dynlint_internal[...]::cargo::{use#15})
        //     |
        //     = help: This is a known issue with the compiler. Run `cargo clean -p dynlint_internal` or `cargo clean` to allow your project to compile

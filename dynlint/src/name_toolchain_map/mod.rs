...
use dynlint_internal::{env, parse_path_filename};
...
impl<'opts> Lazy<'opts> {
    #[must_use]
    pub const fn new(opts: &'opts crate::Dynlint) -> Self {
        Self {
            inner: Inner {
                opts,
    ...
    pub fn get_or_try_init(&self) -> Result<&NameToolchainMap> {
        self.inner
            ...
            .get_or_try_init(|| -> Result<_> {
                ...
                let dynlint_library_paths = dynlint_library_paths()?;

                #[cfg(feature = "metadata")]
                let workspace_metadata_packages =
                    crate::metadata::workspace_metadata_packages(self.inner.opts)?;

                for path in dynlint_library_paths {
                    for entry in dynlint_libraries_in(&path)? {
...
fn dynlint_library_paths() -> Result<Vec<PathBuf>> {
    let mut paths = Vec::new();

    if let Ok(val) = env::var(env::DYNLINT_LIBRARY_PATH) {
        for path in split_paths(&val) {
            ensure!(
                path.is_absolute(),
                "DYNLINT_LIBRARY_PATH contains `{}`, which is not absolute",
                path.to_string_lossy()
            );
            ensure!(
                path.is_dir(),
                "DYNLINT_LIBRARY_PATH contains `{}`, which is not a directory",
                path.to_string_lossy()
            );
            paths.push(path);
        }

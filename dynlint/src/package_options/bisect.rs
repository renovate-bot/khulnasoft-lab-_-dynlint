...
use dynlint_internal::{rustup::SanitizeEnvironment, Command};
...
pub fn bisect(opts: &Dynlint, path: &Path, start: &str) -> Result<()> {
    Command::new("cargo")
        .args(["bisect-rustc", "-V"])
        .success()
    ...
    dynlint_internal::cargo::build(&description, opts.quiet)

...
pub fn new_package(opts: &Dynlint, path: &Path) -> Result<()> {
    let name = path
        .file_name()
...
pub fn upgrade_package(opts: &Dynlint, path: &Path) -> Result<()> {
    let rev = {
    ...
    if opts.bisect {
        ...
        dynlint_internal::cargo::update(&description, opts.quiet)
            .sanitize_environment()
            .current_dir(path)
            .success()?;

        if dynlint_internal::cargo::build(&description, opts.quiet)
            .sanitize_environment()
            .current_dir(path)
            .args(["--all-targets"])
            .success()

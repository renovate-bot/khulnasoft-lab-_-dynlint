...
pub fn build() -> Result<()> {
    // smoelius: The examples use `dynlint-link` as the linker, so it must be built first.
    #[allow(unknown_lints, env_cargo_path)]
    crate::cargo::build("dynlint-link", false)
        .sanitize_environment()
        .current_dir(Path::new(env!("CARGO_MANIFEST_DIR")).join("../dynlint-link"))
        .success()?;
